...
- Inventory is not connected. The feed explicitly says `unknown`; structured data omits unverified availability. Confirm inventory and implement a maintained stock source before publishing `in_stock` or enabling agent purchase flows.
...
2. Confirm stock, actual SKU/GTIN if assigned, return shipping costs/conditions, and owner-approved privacy and sales terms. Do not add legal terms or inventory promises by assumption.
